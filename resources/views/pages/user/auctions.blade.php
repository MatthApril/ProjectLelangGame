    ...
    <div class="row mt-4">
            ...
            <div class="col-md-3 mb-4">
                <div class="card h-100 shadow-sm border-0">
                        ...
                        <div class="d-flex align-items-center gap-2">
                            <div class="bg-light rounded-circle border d-flex align-items-center justify-content-center" style="width: 24px; height: 24px;">
                                @if($auction->product->shop->shop_img)
                                    <img src="{{ asset('storage/shops/' . $auction->product->shop->shop_img) }}" alt="Shop Image" class="rounded-circle" style="width: 24px; height: 24px; object-fit: cover;">
                                @else
                                    <i class="bi bi-person-fill text-secondary" style="font-size: 12px;"></i>
                                @endif
                            </div>
                            <small class="text-muted" style="font-size: 13px;">
                                Toko {{ $auction->product->shop->shop_name ?? 'Unknown' }}
                                @auth
                                    {!! Auth::user()->user_id == $auction->product->shop->owner_id ? '<b> (Anda)</b>' : '' !!}
                                @endauth
                            </small>
                        </div>
                    </div>
                    ...
                    <div class="card-footer bg-white border-top-0 pt-0">
                        @auth
                            @if(Auth::user()->user_id == $auction->product->shop->owner_id)
                                <a href="{{ route('seller.auctions.detail', $auction->auction_id) }}" class="btn btn-outline-success w-100 rounded-pill">
        ...
        @empty
            <div class="col-12 text-center py-5">
                <img src="{{ asset('images/product-empty.png') }}" alt="No Auction" width="200" class="mb-3">
                <h5 class="text-muted">Belum ada lelang yang tersedia</h5>
            </div>
        @endforelse
    </div>
