    ...
    <div class="container">
        ...
        <div>
            ...
            @else
                ...
                <div class="card p-3 mb-3">
                    <div class="row">
                        <div class="col-md-3 text-center my-2">
                            <img src="{{ asset('storage/' . $item->product->product_img) }}" alt="{{ $item->product->product_name}}" width="300" class="rounded shadow">
                        </div>
                        <div class="col-md-9 my-2">
                            <h4 class="fw-semibold m-0">{{ $item->product->product_name}}</h4>
                            <p class="text-secondary"><i class="bi bi-grid"></i> {{ $item->product->category->category_name }} | <i class="bi bi-controller"></i> {{ $item->product->game->game_name }}</p>
                            <hr>
                                    ...
                                    <div class="d-flex align-items-center gap-1">
                                        <div>
                                            @if ($item->product->shop->shop_img)
                                                <img 
                                                    src="{{ asset('storage/' . $item->product->shop->shop_img) }}" 
                                                    alt="" 
                                                    class="shop-avatar"
                                                >
                                            @else
                            ...
                            <p class="m-0">Harga Satuan : <strong>Rp{{ number_format($item->product_price, 0, ',', '.') }}</strong></p>
                            <p class="m-0">Subtotal : <strong>Rp{{ number_format($item->subtotal, 0, ',', '.') }}</strong></p>
                            <p class="m-0">
                                Status :
                                <strong class="{{ $item->status === 'completed' ? 'text-success' : '' }}">
                                    {{ ucfirst($item->status) }}
                                </strong>
                            </p>
                            <div id="review-section-{{ $item->order_item_id }}" class="mt-2">
                                @if ($item->status === 'completed')
                                    ...
                                    @else
                                        <button type="button" data-bs-toggle="modal" data-bs-target="#reviewModal-{{ $item->order_item_id }}" class="btn btn-outline-primary btn-sm"><i class="bi bi-star"></i> Beri Review</button>
                                    @endif
                                @endif
                            </div>
                        </div>
                ...
                <!-- Modal Review -->
                <div class="modal fade" id="reviewModal-{{ $item->order_item_id }}" tabindex="-1" aria-labelledby="reviewModalLabel-{{ $item->order_item_id }}" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h1 class="modal-title fs-5 fw-bold" id="reviewModalLabel-{{ $item->order_item_id }}">Berikan Review Anda!</h1>
                            ...
                            <form class="reviewForm" data-order-item-id="{{ $item->order_item_id }}">
                                ...
                                <div class="modal-body">
                                    ...
                                    <div class="d-flex gap-2 mb-3">
                                        @for ($i = 1; $i <= 5; $i++)
                                            <input type="radio" class="btn-check" name="rating-{{ $item->order_item_id }}" id="rating{{ $i }}-{{ $item->order_item_id }}" value="{{ $i }}" required>
                                            <label class="btn btn-outline-warning" for="rating{{ $i }}-{{ $item->order_item_id }}">
                                    ...
                                    <textarea name="comment" id="comment-{{ $item->order_item_id }}" rows="4" maxlength="1000" class="form-control mb-2" placeholder="Berikan Komentar Tentang Produk Disini"></textarea>
                                    <div class="text-danger" id="error-comment-{{ $item->order_item_id }}"></div>
                                </div>
                                <div class="modal-footer">
                                    <button type="submit" class="btn btn-success">Kirim <i class="bi bi-caret-right-fill"></i></button>
                                </div>
                            </form>
                        </div>
                    </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            document.querySelectorAll('.reviewForm').forEach(form => {
                form.addEventListener('submit', function(e) {
                    ...
                    const ratingInput = document.querySelector(`input[name="rating-${orderItemId}"]:checked`);
                    ...
                    // Validasi
                    if (!ratingInput) {
                        document.getElementById(`error-rating-${orderItemId}`).textContent = 'Pilih rating terlebih dahulu!';
                        return;
                    }
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            const modal = bootstrap.Modal.getInstance(document.getElementById(`reviewModal-${orderItemId}`));
                            modal.hide();
                            setTimeout(() => {
                                location.reload();
                            }, 1000);
