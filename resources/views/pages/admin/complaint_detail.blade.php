...
<div>
    ...
    <div>
        ...
        <img src="{{ asset('storage/' . $complaint->proof_img) }}" alt="Bukti Buyer" onclick="window.open(this.src, '_blank')">
        ...
        <div>
            ...
            @if($complaint->response->attachment)
                ...
                @if(in_array($ext, ['jpg', 'jpeg', 'png']))
                    <img src="{{ asset('storage/' . $complaint->response->attachment) }}" alt="Lampiran Seller" onclick="window.open(this.src, '_blank')">
                    <br><small>Klik gambar untuk memperbesar</small>
                @else
    ...
    @if($complaint->status === 'waiting_admin' && $complaint->status !=='resolved')
        <div>
            ...
            <div>
                ...
                <ul>
                    <li><strong>Refund:</strong> Buyer akan menerima Rp {{ number_format($complaint->orderItem->subtotal, 0, ',', '.') }} kembali</li>
                    <li><strong>Refund:</strong> running transactions seller akan dikurangi Rp {{ number_format($complaint->orderItem->subtotal, 0, ',', '.') }}</li>
            ...
            <form action="{{ route('admin.complaints.resolve', $complaint->complaint_id) }}" method="POST">
                ...
                <button type="submit" onclick="return confirm('YAKIN PUTUSKAN KOMPLAIN INI?\n\nKeputusan bersifat FINAL dan akan langsung dieksekusi!')">
                    PUTUSKAN KOMPLAIN
                </button>
            </form>
        </div>
