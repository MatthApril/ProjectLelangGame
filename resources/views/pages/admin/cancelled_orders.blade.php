...
<div>
    ...
    <form method="GET" action="{{ route('admin.cancelled_orders.index') }}">
        ...
        <select name="refund_status" onchange="this.form.submit()">
            ...
            <option value="refunded" {{ request('refund_status') === 'refunded' ? 'selected' : '' }}>Sudah Refund</option>
            <option value="not_refunded" {{ request('refund_status') === 'not_refunded' ? 'selected' : '' }}>Belum Refund</option>
        </select>
    </form>
                ...
                <tr>
                    ...
                    <td>
                        @if($item->product->product_img)
                            <img src="{{ asset('storage/' . $item->product->product_img) }}" width="50" alt="">
                        @endif
