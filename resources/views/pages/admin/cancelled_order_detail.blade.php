    ...
    <div class="refund-action-section">
        ...
        @if($orderItem->is_refunded)
            <div style="background: lightgreen; padding: 15px; border: 2px solid green;">
                ...
                <p>Buyer sudah menerima refund sebesar: <strong>Rp {{ number_format($orderItem->subtotal, 0, ',', '.') }}</strong></p>
            </div>

            <form action="{{ route('admin.cancelled_orders.undo_refunded', $orderItem->order_item_id) }}" method="POST" style="margin-top: 15px;">
                @csrf
                <button type="submit" style="background: orange; color: white; padding: 10px 20px;" onclick="return confirm('Batalkan status refund? Ini untuk keperluan koreksi data.')">
        ...
        @else
            <div style="background: lightyellow; padding: 15px; border: 2px solid orange;">
                ...
                <p>Jumlah yang harus direfund: <strong>Rp {{ number_format($orderItem->subtotal, 0, ',', '.') }}</strong></p>
            </div>

            <form action="{{ route('admin.cancelled_orders.mark_refunded', $orderItem->order_item_id) }}" method="POST" style="margin-top: 15px;">
                @csrf
                <button type="submit" style="background: green; color: white; padding: 10px 20px;" onclick="return confirm('Tandai pesanan ini sebagai REFUNDED?\n\nIni hanya untuk keperluan tracking admin.')">
